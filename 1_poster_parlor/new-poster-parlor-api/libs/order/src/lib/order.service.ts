...
export class OrdersService {
  ...
  public async validateOrderItems(
    ...
    for (const item of items) {
      ...
      validatedItems.push({ posterId: item.posterId, quantity: item.quantity, price: poster.price, });
  ...
  public async createOrder(orderDetail: CreateOrderDto, paymentInfo?: { razorpayPaymentId?: string; razorpayOrderId?: string }): Promise<OrderDocument> {
    try {
      // ------------------ ID VALIDATION ------------------
      if (
  ...
  public async getOrdersByUserId(userId: string, page: number = 1, limit: number = 10): Promise<PaginatedOrdersResponse> {
